...
use crate::models::{compute_quality_score, AgentRun, ClaudeSession, ClaudemdAnalysis, TokenUsage};
...
fn score_claudemd(content: &str) -> u8 {
    ...
    let length_score: u32 = if words >= 300 {
        30
    } else if words >= 150 {
        23
    } else if words >= 80 {
    ...
    let structure_score =
        (headings * 5).min(15) + ((code_delimiters / 2) * 5).min(10) + (bullets / 4).min(5);
    ...
    let content_score: u32 = categories
        .iter()
        .map(|kws| {
...
pub fn score_claudemd_detailed(content: &str) -> ClaudemdAnalysis {
    ...
    let length_score: u32 = if word_count >= 300 {
        30
    ...
    if !has_build {
        suggestions.push("Add build/compile commands");
    }
    if !has_tests {
        suggestions.push("Add test instructions");
    }
    if !has_run {
        suggestions.push("Add run/launch instructions");
    }
    if !has_commands {
        suggestions.push("Add common command examples");
    }
    if !has_structure {
        suggestions.push("Describe project structure");
    }
    if !has_conventions {
        suggestions.push("Document coding conventions");
    }
    if word_count < 80 {
        suggestions.push("Expand content (too brief)");
    }
    if heading_count == 0 {
...
pub fn find_claudemd_path(project_path: &str) -> Option<std::path::PathBuf> {
    ...
    let mut queue: VecDeque<(std::path::PathBuf, u32)> = VecDeque::from([(start.to_path_buf(), 0)]);
        ...
        if entry_type == "assistant" {
            if let Some(msg) = val.get("message") {
                ...
                if let Some(usage) = msg.get("usage") {
                    ...
                    let day: NaiveDate = turn_time.unwrap_or_else(Local::now).date_naive();
...
pub fn parse_agents(session_path: &Path) -> Vec<AgentRun> {
    ...
    for line in content.lines() {
        ...
        if entry_type == "assistant" {
            ...
            for block in blocks {
                ...
                if block_type != "tool_use" || block_name != "Agent" {
                    continue;
                }
        ...
        if entry_type == "user" {
            ...
            for block in blocks {
                ...
                let Some(pending_agent) = pending.remove(&tool_use_id) else {
                    continue;
                };
    ...
    for agent in &mut completed {
        let Some(ref agent_id) = agent.agent_id else {
            continue;
        };
...
mod tests {
    ...
    fn temp_file(name: &str) -> PathBuf {
        let uniq = SystemTime::now()
            ...
            .unwrap_or_default()
            .as_nanos();
        let dir = std::env::temp_dir().join("claux-tests");
        let _ = fs::create_dir_all(&dir);
        dir.join(format!("{}-{}.jsonl", name, uniq))
    }

    #[test]
    fn parse_session_rejects_malformed_only_logs() {
        let path = temp_file("malformed");
        fs::write(&path, "{not-json}\n---\n").unwrap();
        let ids = HashSet::new();
        let parsed = parse_session(&path, &ids, false);
        assert!(parsed.is_err());
    }

    #[test]
    fn parse_session_accepts_partial_with_usage() {
        let path = temp_file("partial");
        let line = r#"{"timestamp":"2026-06-01T10:00:00Z","type":"assistant","message":{"model":"claude-sonnet-4-6","usage":{"input_tokens":10,"output_tokens":3,"cache_read_input_tokens":0,"cache_creation_input_tokens":0}}}"#;
